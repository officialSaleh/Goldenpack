...
export const POS: React.FC = () => {
  ...
  return (
    <div className="h-full relative overflow-hidden flex flex-col bg-slate-50/50">
      {/* Main Product Discovery Area */}
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Sticky Search & Category Bar */}
        <div className="sticky top-0 z-30 p-4 lg:p-6 bg-white/90 backdrop-blur-xl border-b border-slate-200/60 shadow-sm space-y-4">
          <div className="flex items-center space-x-3">
            <div className="flex-1 relative group">
              ...
              <input 
                ...
                className="w-full pl-12 pr-4 py-3.5 bg-slate-100/50 border border-slate-200 rounded-2xl focus:bg-white focus:ring-4 focus:ring-brand-gold/10 focus:border-brand-gold outline-none transition-all font-bold text-slate-900 text-sm"
                value={searchTerm}
        ...
        {/* Product Grid - 1 Column on Mobile, 2 Columns on Laptop/Desktop */}
        <div className="flex-1 overflow-y-auto p-4 lg:p-6 grid grid-cols-1 md:grid-cols-2 gap-4 lg:gap-6 pb-32 scrollbar-hide">
      ...
      <button 
        ...
        className={`fixed bottom-8 right-8 z-40 w-16 h-16 lg:w-20 lg:h-20 bg-brand-dark text-brand-gold rounded-full flex items-center justify-center shadow-2xl hover:scale-110 active:scale-95 transition-all group ${cart.length > 0 ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0 pointer-events-none'}`}
      >
        <ShoppingCart size={28} />
        {cartItemCount > 0 && (
          <span className="absolute -top-1 -right-1 w-7 h-7 bg-brand-gold text-brand-dark text-[10px] font-black rounded-full flex items-center justify-center border-4 border-brand-dark animate-in zoom-in-50">
            {cartItemCount}
          </span>
        )}
      </button>
      ...
      {isCartOpen && (
        <div 
          className="fixed inset-0 bg-brand-dark/40 backdrop-blur-sm z-50 animate-in fade-in duration-300"
          onClick={() => setIsCartOpen(false)}
        />
      )}

      {/* Cart Drawer Panel */}
      <div className={`fixed top-0 right-0 h-full w-full max-w-md bg-white z-[60] shadow-2xl transition-transform duration-500 ease-out border-l border-slate-100 flex flex-col ${isCartOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 bg-brand-dark text-brand-gold rounded-xl flex items-center justify-center">
              <ShoppingCart size={20} />
            </div>
            <h3 className="text-xl font-black text-brand-dark tracking-tight uppercase italic leading-none">Checkout <span className="text-brand-gold">Queue</span></h3>
          </div>
          <button onClick={() => setIsCartOpen(false)} className="w-10 h-10 rounded-xl hover:bg-slate-200 flex items-center justify-center transition-colors">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4 scrollbar-hide">
          {cart.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-slate-300 space-y-4">
              ...
              <p className="font-black uppercase tracking-widest text-[10px]">Your queue is empty</p>
            </div>
          ) : (
            cart.map(item => (
              <div key={item.productId} className="flex items-center space-x-4 p-4 bg-slate-50 rounded-3xl border border-slate-100 group">
                <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-brand-dark shadow-sm shrink-0">
                  <Package size={20} />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-black text-brand-dark truncate uppercase">{item.productName}</p>
                  <p className="text-[10px] font-bold text-slate-400">{db.formatMoney(item.price)} / unit</p>
                </div>
          ...
          <div className="p-6 bg-slate-50 border-t border-slate-100 space-y-6">
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="font-black text-brand-dark">{db.formatMoney(cartTotal)}</span>
              </div>
              <div className="flex justify-between items-center">
                <div className="flex items-center space-x-2">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">VAT (5%)</span>
                  <button onClick={() => setVatEnabled(!vatEnabled)} className={`w-8 h-4 rounded-full transition-colors relative ${vatEnabled ? 'bg-brand-gold' : 'bg-slate-300'}`}>
                    <div className={`absolute top-0.5 w-3 h-3 bg-white rounded-full transition-all ${vatEnabled ? 'left-4.5' : 'left-0.5'}`} />
                  </button>
                </div>
                <span className="font-black text-brand-dark">{db.formatMoney(vatEnabled ? cartTotal * VAT_RATE : 0)}</span>
              </div>
              <div className="pt-3 border-t border-slate-200 flex justify-between items-center">
                <span className="text-xs font-black text-brand-dark uppercase tracking-widest">Total Payable</span>
                <span className="text-2xl font-black text-brand-dark tracking-tighter italic">
                  {db.formatMoney(cartTotal + (vatEnabled ? cartTotal * VAT_RATE : 0))}
                </span>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <h4 className="text-[10px] font-black text-brand-gold uppercase tracking-widest">Customer Assignment</h4>
                {selectedCustomer && (
                  <button onClick={() => setSelectedCustomer(null)} className="text-[10px] font-black text-rose-500 uppercase">Clear</button>
                )}
              </div>
              
              {selectedCustomer ? (
                <div className="p-4 bg-brand-dark text-white rounded-2xl flex items-center justify-between group">
                  <div className="flex items-center space-x-3">
                    <div className="w-10 h-10 bg-brand-gold text-brand-dark rounded-xl flex items-center justify-center font-black">
                      {selectedCustomer.name.charAt(0)}
                    </div>
                    <div>
                      <p className="text-xs font-black uppercase tracking-tight">{selectedCustomer.name}</p>
                      <p className="text-[9px] font-bold text-brand-gold/80">Balance: {db.formatMoney(selectedCustomer.outstandingBalance)}</p>
                    </div>
                  </div>
                  <button onClick={() => setShowLedgerModal(true)} className="p-2 hover:bg-white/10 rounded-lg transition-colors">
                ...
                <div className="relative group">
                  ...
                  <select 
                    className="w-full pl-12 pr-4 py-4 bg-white border border-slate-200 rounded-2xl font-black text-brand-dark text-xs appearance-none outline-none focus:ring-4 focus:ring-brand-gold/10 transition-all"
                    onChange={(e) => {
      ...
      <Modal isOpen={showConfirmModal} onClose={() => setShowConfirmModal(false)} title="Operational Verification">
        <div className="space-y-8">
          <div className="bg-slate-50 p-8 rounded-[40px] border border-slate-100 flex flex-col items-center">
            ...
            <div className="w-full mt-10 pt-10 border-t border-slate-200 flex items-end justify-between">
              <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Final Valuation</span>
              <span className="text-4xl font-black text-brand-gold tracking-tighter">{db.formatMoney(cartTotal + (vatEnabled ? cartTotal * VAT_RATE : 0))}</span>
            </div>
          </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {(['Cash', 'Bank Transfer', 'Credit'] as const).map(type => (
                <button 
                  ...
                  className={`py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest border-2 transition-all ${paymentType === type ? 'bg-brand-gold text-brand-dark border-brand-gold shadow-xl shadow-brand-gold/20' : 'border-slate-100 text-slate-400 hover:border-slate-200'}`}
                >
