...
export const Layout: React.FC<LayoutProps> = ({ children, activeTab, setActiveTab }) => {
  ...
  return (
    <div className="flex h-screen bg-brand-bg overflow-hidden relative">
      ...
      {/* Mobile Top Bar */}
      <div className="lg:hidden fixed top-0 left-0 right-0 bg-brand-dark/95 backdrop-blur-md border-b border-white/5 px-6 py-4 flex items-center justify-between z-50">
        ...
        <button 
          ...
          className="p-2 bg-brand-gold text-brand-dark rounded-xl shadow-lg shadow-brand-gold/20"
      ...
      {/* Mobile Menu Overlay */}
      <div 
        className={`lg:hidden fixed inset-0 bg-brand-dark/60 backdrop-blur-sm z-50 transition-opacity duration-300 ${isMobileMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`} 
        onClick={() => setIsMobileMenuOpen(false)}
      >
        <aside 
          className={`w-72 h-full bg-brand-dark shadow-2xl p-8 flex flex-col transform transition-transform duration-300 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`} 
          onClick={e => e.stopPropagation()}
        >
          <div className="flex items-center justify-between mb-10">
             <h1 className="text-xl font-black text-brand-gold uppercase italic tracking-tighter">Menu</h1>
            ...
            {filteredNav.map((item) => (
              <button
                ...
                className={`w-full flex items-center space-x-4 px-6 py-4 rounded-2xl transition-all ${
                  activeTab === item.id ? 'bg-brand-gold text-brand-dark font-bold' : 'text-gray-400 hover:text-brand-gold'
                }`}
              >
                {item.icon}
          ...
          <button 
            onClick={handleLogout}
            className="mt-6 flex items-center space-x-4 px-6 py-4 text-rose-400 font-bold border-t border-white/5 pt-8"
          >
            <LogOut size={20} />
            <span>Logout</span>
          </button>
      ...
      <main className="flex-1 overflow-y-auto w-full">
        <div className="pt-20 lg:pt-0 max-w-7xl mx-auto p-4 md:p-8 lg:p-10">
          {children}
        </div>
      </main>
    </div>
